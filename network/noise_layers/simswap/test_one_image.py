...
class SimSwap(nn.Module):
    def __init__(self, temp="temp/", target="/mnt/c/Users/bob13/fake/dataset/celeba_256/val/"):
    ...
    def get_target(self):
        while True:
            ...
            path = self.target + str(idx).zfill(6) + ".png"
    ...
    def forward(self, image_cover_mask):
        ...
        for i in range(image.shape[0]):
            ...
            #########################################################
            pic_a = self.get_target()           # non-image
            
            img_a = Image.open(pic_a).convert('RGB')
            img_a = self.transformer_Arcface(img_a)
            img_id = img_a.view(-1, img_a.shape[0], img_a.shape[1], img_a.shape[2])     # non-given image

            img_b = self.transformer(im)
            img_att = img_b.view(-1, img_b.shape[0], img_b.shape[1], img_b.shape[2])    # given image

            # convert numpy to tensor
            img_id = img_id.cuda()              # non-given image
            img_att = img_att.cuda()            # given image
            ...
            ############## Forward Pass ######################
            img_fake = self.model(img_att, img_id, latend_id, latend_id, True)      # img_att <-> img_id
